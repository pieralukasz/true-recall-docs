...
1. **Action bar** — Note type picker, source note picker, format help
...
- **Note type picker** — Dropdown to select which note type to create: Basic, Cloze, or any [custom note type](/creation/custom-note-types/) you've defined. Import Studio remembers your last selection between sessions.
- **Source note picker** — Combobox with search to link created cards to a specific note in your vault. If the selected note doesn't have a `flashcard_uid` yet, one is generated automatically. Also remembers your last selection.
- **Help icon** — Opens a quick reference showing the expected input format for the selected note type.
...
| `Cmd/Ctrl + 3` | Insert a blank card template for the selected note type |

The template shortcut inserts a ready-made block with the correct `#type/<slug>` tag and field names — useful when you can't remember the exact field names for a note type.

### Card Preview

A live preview of all parsed cards appears below the editor. The preview updates about 150 milliseconds after you stop typing.
...
| **Copy Prompt** | Copies an AI-ready prompt with the selected note type's field names to your clipboard |
| **Save** | Creates all parsed cards in batch. Disabled when there are 0 valid cards or no source note selected |

## Supported Formats

Import Studio auto-detects four input formats as you type. The format indicator in the footer shows which format is currently active.
...
Columns map to the selected note type's fields in order — first column to the first field, second column to the second, and so on. Import Studio auto-detects this format when more than half of the non-empty lines contain tab characters.
...
:::tip[Paste from spreadsheets]
Copy columns from Excel or Google Sheets — they paste as tab-separated text, which Import Studio picks up automatically. Two columns for Basic cards, or more columns for custom note types with additional fields.
:::

### Double-Colon

The simplest format for two-field note types. Separate front and back with ` :: ` (spaces around the double colon).
...
If cloze patterns like `{{c1::text}}` appear in the text, Import Studio automatically creates cloze cards instead of basic cards.
...
| Tab-separated | >50% of non-empty lines contain tabs | Spreadsheet pastes, bulk vocabulary lists |
| Double-colon | Lines contain ` :: ` separator | Quick two-field cards |
| Cloze | Lines contain `{{c1::...}}` patterns | Fill-in-the-blank cards |

## Creating Cards

1. Open Import Studio from the Command Palette ("Add flashcards")
2. Select a **note type** from the action bar
3. Optionally pick a **source note** to link the cards to
4. Type or paste formatted text into the editor
5. Watch the **Card Preview** update with parsed cards
6. Review the preview — check card count and content
7. Click **Save** or press `Cmd/Ctrl + Enter`
...
Only cards created with block format that include a `<!-- source: -->` comment will highlight text in your notes when hovered in the [Flashcard Panel](/views/flashcard-panel/). Tab-separated and double-colon formats don't support source comments — cards created with these formats won't have hover-highlighting.
:::

## Using AI to Generate Input

The **Copy Prompt** button in the footer generates a ready-made prompt containing the selected note type's field names and formatting instructions. Use it with any external AI tool to produce properly formatted cards.

1. Select a note type in Import Studio
2. Click **Copy Prompt** in the footer
3. Paste the prompt into ChatGPT, Claude, or another AI tool
4. Add your study material to the prompt
5. Copy the AI's output
6. Paste it back into Import Studio — the format is auto-detected
7. Review the preview and save

The generated prompt includes block format examples with the correct field names, rules for atomic facts, and guidelines for bold key terms and Obsidian `[[wikilinks]]`.
...
Press `Cmd/Ctrl + 3` to insert a block format template for the currently selected note type. The template includes the correct `#type/<slug>` tag and all field names — just fill in the values.
...
| **Formats** | Block, tab, double-colon, cloze | Form-based (no syntax) |
