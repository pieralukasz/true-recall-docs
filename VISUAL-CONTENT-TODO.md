...
## Capture method (automated via Obsidian CLI)

Screenshots are produced by driving a clean **demo vault** with the Obsidian 1.12.7+ CLI (`dev:screenshot`). Full reproducible procedure: [`docs/screenshot-capture-e2e.md`](./docs/screenshot-capture-e2e.md). Maximized window, dark theme, right sidebar collapsed for full-area views, cards generated by the real AI Assistant (Pro) so views show representative data — never the author's personal vault.

### Done (embedded)

| Screenshot | Page | Asset |
|---|---|---|
| Dashboard | `views/dashboard.md` | `screenshots/dashboard/dashboard.png` |
| Statistics | `views/statistics.md` | `screenshots/stats/statistics.png` |
| Card Browser | `views/card-browser.md` | `screenshots/browser/card-browser.png` |
| Flashcard Panel | `views/flashcard-panel.md` | `screenshots/panel/flashcard-panel.png` |
| FSRS Simulator | `views/fsrs-simulator.md` | `screenshots/simulator/fsrs-simulator.png` |
| AI Inbox | `views/ai-inbox.md` | `screenshots/assistant/ai-inbox.png` |
| Note Type Manager | `creation/custom-note-types.md` | `screenshots/note-types/note-type-manager.png` |

### Still open (not yet captured)

Interactive / tier-specific flows that need extra driving or setup: Ask AI / ThreadWorkspace (generation applies directly — the composer/draft UI wasn't captured), Review loop + Session Complete, Type-in grading, Image Occlusion editor, Knowledge Chat (command didn't register without index setup), Selection Toolbar, and the review activity heatmap (fresh vault has no history). The remaining `<!-- TODO PHOTO -->` markers below track these.
